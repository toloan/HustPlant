...
public class PathFinder {
  ...
  public List<Point> findPath(Point start, Point end) {
    ...
    while (!openSet.isEmpty()) {
      ...
      for (int i = Math.max(0, current.x - 1); i <= Math.min(mWidth - 1, current.x + 1); i++) {
        for (int j = Math.max(0, current.y - 1); j <= Math.min(mHeight - 1, current.y + 1); j++) {
          ...
          if (!closedSet.contains(neighbor))
            fScore.insertOrUpdate(tentativeGScore + distance(neighbor, end) + bonus, neighbor);
        }
      }
    }
    return null;
